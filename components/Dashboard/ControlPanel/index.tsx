...
export default function ControlPanel () {
  const { reset, switchChancesPanel, showPanel, threeChoiceType, threeChoice1, threeChoice2, threeChoice3, histBoards, restoreHistBoard, gameStatus } = useModel('useGameModel', (model) => pick(model, 'reset', 'switchChancesPanel', 'showPanel', 'threeChoiceType', 'threeChoice1', 'threeChoice2', 'threeChoice3', 'histBoards', 'restoreHistBoard', 'gameStatus'))
  ...
  return (
		<div className='bg-green-800 w-[350px] ml-[24px] border border-solid border-green-600'>
				...
				{(histBoards.length === 0 || gameStatus === GAME_STATUS.PASS)
				  ? null
				  : <div className='flex justify-left ml-6'>
							<button
								className={classnames('px-4 py-1 my-4 text-sm text-green-50 font-normal rounded border border-green-200 hover:text-white hover:bg-green-400 hover:border-transparent focus:outline-none w-[130px]')}
								onClick={() => { restoreHistBoard() }}
							>
									{'回退一步 (' + histBoards.length + ')'}
							</button>
					  </div>
				}
		</div>
